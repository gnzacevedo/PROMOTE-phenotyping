...
if(exists('sce.s')){
  if(!'UMAP' %in% reducedDimNames(sce.s)){
...
koi.signif <- c(6,13,23,26)
koi.trend <- c(25)

split.hm <- umap.df.s %>%
  select(meta28, all_of(fts)) %>%
  mutate(koi=ifelse(meta28 %in% koi.signif, 'Sig.',
                    ifelse(meta28 %in% koi.trend, 'Trend',
                           'None'))) %>%
  group_by(meta28, koi) %>%
  summarize(across(everything(), median)) %>%
  pivot_longer(cols=where(is.numeric), names_to='Marker', values_to='Median.expr') %>%
...
plist <- lapply(dim.pairs,
                  FUN=function(d){ 
                    plotBivariates.highlightClusters(umap.df.s, dimpair=d, meta='meta28', koi=c(koi.signif), bg.evts=1e5)
                    })
