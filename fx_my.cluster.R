my.cluster <- function (x, features = "type", xdim = 10, ydim = 10, maxK = 20, 
          verbose = TRUE, seed = 1) 
{
  require(FlowSOM)
  ...
  fsom <- FlowSOM::ReadInput(my.flowFrame(t(assay(x, "exprs"))))
  som <- FlowSOM::BuildSOM(fsom, colsToUse = features, silent = TRUE, 
                  xdim = xdim, ydim = ydim)
  if (verbose) 
    message("o running ConsensusClusterPlus metaclustering...")
  pdf(NULL)
  mc <- suppressWarnings(suppressMessages(ConsensusClusterPlus:::ConsensusClusterPlus(t(som$map$codes), 
...
my.flowFrame <- function (exprs, parameters, description = list()) 
{
  ...
  if (missing(parameters)) 
    parameters <- flowCore:::parDefault(exprs)
  else flowCore:::isValidParameters(parameters, exprs)
  flowCore:::checkClass(description, "list")
  fr <- new("flowFrame", exprs = exprs, parameters = parameters, 
            description = description)
  tmp <- tempfile()
  on.exit(unlink(tmp))
  suppressMessages(write.FCS(fr, tmp))
  suppressMessages(read.FCS(tmp, truncate_max_range=FALSE))
}
